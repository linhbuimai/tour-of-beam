...
class WordsAlphabet:
  ...
  def __str__(self) -> str:
    return "WordsAlphabet(alphabet:'%s', fruit='%s', country='%s')" \
      % (self.alphabet, self.fruit, self.country)
...
def apply_transform(fruits, countries):
  ...
  def cogbk_result_to_wordsalphabet(cgbk_result):
    ...
    return WordsAlphabet(alphabet,
                         words['fruits'][0],
                         words['countries'][0])
  
  fruits_kv = (
    fruits | 'Fruit to KV' >> beam.Map(map_to_alphabet_kv)
  )
  countries_kv = (
    countries | 'Country to KV' >> beam.Map(map_to_alphabet_kv)
  )
